...
Invocation semantics (plan §Phase 3, §7-8):
  python slm_from_scratch/scripts/model.py            → unit tests + artifact dump
  python slm_from_scratch/scripts/model.py --smoke-test → unit tests only, no writes
...
Outputs (written by __main__ on test success, skipped under --smoke-test):
  ...
  slm_from_scratch/configs/trainable_params.json   — parameter count breakdown
