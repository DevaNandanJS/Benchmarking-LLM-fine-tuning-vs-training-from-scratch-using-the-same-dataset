...
Inputs (all produced by earlier phases on Colab):
  slm_from_scratch/checkpoints/best_val/<run_name>/best_val.pt
  slm_from_scratch/checkpoints/best_val/<run_name>/best_val_config.json
  slm_from_scratch/eval/sweep_results.csv
...
Outputs:
  slm_from_scratch/eval/loss_curve.png
  slm_from_scratch/eval/final_metrics.json
  slm_from_scratch/eval/loss_curve_interpretation.md

Run on Colab (from repo root after git pull):
  !python slm_from_scratch/scripts/eval.py
  !python slm_from_scratch/scripts/eval.py --run base   # force a specific run
...
def load_best_checkpoint(run_name: str, device):
    ...
    if not ckpt_dir.exists():
        raise FileNotFoundError(
            ...
            f"Run `!python slm_from_scratch/scripts/train.py --run {run_name}` on Colab first."
        )
