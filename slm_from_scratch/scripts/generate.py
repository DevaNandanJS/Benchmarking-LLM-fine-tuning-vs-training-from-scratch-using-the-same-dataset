...
Prompts are taken verbatim from finetuning_model/generations/finetuning_samples.md
so the comparison is controlled on inputs.

Outputs:
    slm_from_scratch/generations/slm_samples.md  — 8 prompts × 2 modes each

Run on Colab (from repo root after git pull):
    !python slm_from_scratch/scripts/generate.py
    !python slm_from_scratch/scripts/generate.py --run base  # force a specific run
