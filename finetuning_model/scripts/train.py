...
Sweep design:
    Three independent runs are launched separately from the notebook:
        !python finetuning_model/scripts/train.py --run r4
        !python finetuning_model/scripts/train.py --run r8
        !python finetuning_model/scripts/train.py --run r16
...
Outputs (relative to repo root):
    finetuning_model/configs/run_phase5_<run_name>.json   — run config dump
    finetuning_model/logs/<run_name>/metrics.jsonl         — step-level logging
    finetuning_model/checkpoints/best_val/<run_name>/      — best adapter
    finetuning_model/eval/sweep_results.csv                — appended after each run

Run on Colab (from repo root after git pull):
    !python finetuning_model/scripts/train.py --run r8   # or r4, r16
