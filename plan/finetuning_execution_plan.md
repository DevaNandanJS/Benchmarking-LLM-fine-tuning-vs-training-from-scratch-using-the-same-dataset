...
**Companion document:** `track2_scratch_slm_execution_plan.md`. The two pipelines deliberately share conventions (directory layout, chunking approach, logging format, evaluation metrics) so their outputs can be compared fairly at the end — see Phase 8 in both documents.
...
- **Shared directory layout** (create at the start of Phase 1):
  ...
  llm_task/
    ...
    track1_finetune/
      ...
      track1_run.ipynb        # thin orchestrator notebook — cells sync + call into scripts/
    track2_scratch/
      ...
...
**Recommended pattern:** keep all real logic in importable `.py` modules under `track1_finetune/scripts/` (as the rest of this plan already assumes) and add one thin orchestrator notebook, `track1_finetune/track1_run.ipynb`, whose cells only sync code and call into those scripts:
```python
...
2. Open/create `track1_finetune/track1_run.ipynb`, use the kernel picker (top-right of the notebook) → **Select Another Kernel → Colab → New Colab Server** (or **Auto Connect** to reattach to a recent session — note this silently starts a *new* server if the old one expired, so re-verify GPU/files afterward) → choose **GPU** → **T4** on the free tier, and sign in with your Google account when prompted.
...
5. Install pinned dependencies **inside a notebook cell**, not your local terminal — installing locally puts packages where nothing GPU-relevant runs:
   ...
   ```
6. Record `!pip freeze > track1_finetune/environment.txt` from the same session, then get this file back into the version-controlled repo (commit it from the remote via git, or copy it back) — it's a reproducibility artifact and should live with the code, not just on the ephemeral Colab VM.
...
- [ ] Orchestrator notebook (`track1_run.ipynb`) exists with a working sync cell, verified to pull in a test local edit.
...
5. Save processed dataset to `data/processed/track1_train.pt` / `track1_val.pt` (or `datasets` `save_to_disk`).
...
3. Save prompts + completions to `generations/track1_samples.md` in a readable prompt→completion format.
...
**Definition of Done:**
- [ ] `generations/track1_samples.md` contains ≥5 prompt/completion pairs with brief annotations.
