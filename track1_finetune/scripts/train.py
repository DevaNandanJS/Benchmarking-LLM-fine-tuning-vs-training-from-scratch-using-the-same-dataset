...
Sweep design:
    Three independent runs are launched separately from the notebook:
        !python track1_finetune/scripts/train.py --run r4
        !python track1_finetune/scripts/train.py --run r8
        !python track1_finetune/scripts/train.py --run r16
...
Outputs (relative to repo root):
    track1_finetune/configs/run_phase5_<run_name>.json   — run config dump
    track1_finetune/logs/<run_name>/metrics.jsonl         — step-level logging
    track1_finetune/checkpoints/best_val/<run_name>/      — best adapter
    track1_finetune/eval/sweep_results.csv                — appended after each run

Run on Colab (from repo root after git pull):
    !python track1_finetune/scripts/train.py --run r8   # or r4, r16
