...
Inputs (all produced by earlier phases on Colab):
  track2_scratch/checkpoints/best_val/<run_name>/best_val.pt
  track2_scratch/checkpoints/best_val/<run_name>/best_val_config.json
  track2_scratch/eval/sweep_results.csv
...
Outputs:
  track2_scratch/eval/loss_curve.png
  track2_scratch/eval/final_metrics.json
  track2_scratch/eval/loss_curve_interpretation.md

Run on Colab (from repo root after git pull):
  !python track2_scratch/scripts/eval.py
  !python track2_scratch/scripts/eval.py --run base   # force a specific run
...
def load_best_checkpoint(run_name: str, device):
    ...
    if not ckpt_dir.exists():
        raise FileNotFoundError(
            ...
            f"Run `!python track2_scratch/scripts/train.py --run {run_name}` on Colab first."
        )
